                ...
                if attivita_problematiche:
                    ...
                    st.success("### ✅ Nessuna attività problematica rilevata")
                            ...
                            if attivita['type'] == "Alimentazione":
                                ...
                                for cibo in cibi.split(','):
                                    ...
                                    if cibo in NUTRITION_DB:
                                        ...
                                        if score > 2:
                                            cibi_problematici.append(cibo)
                                        st.write(f"- {cibo}: punteggio {score}")
                                    else:
                                        st.write(f"- {cibo}: NON nel database")
                                
                                st.write(f"**Punteggio totale:** {punteggio_infiammatorio}")
                                if punteggio_infiammatorio > 3:
                            ...
                            # ANALISI ALLENAMENTI
                                ...
                                st.write(f"**Intensità rilevata:** {intensita}")
                                ...
                                if any(p in intensita for p in ['intensa', 'massimale', 'duro', 'pesante', 'intenso']):
                                    ...
                                    if avg_rmssd_totale < 50:
                                        ...
                                        st.success("✅ RMSSD sufficiente per intensità")
                                else:
                                    st.info("ℹ️ Intensità non riconosciuta come 'intensa'")
                            
                            # ANALISI SONNO
                            elif attivita['type'] == "Sonno":
                                durata_sonno = attivita['duration'] / 60
                                if durata_sonno < 6:
                                    data_sonno = attivita['start_time'].strftime('%d/%m')
                                    problema = f"😴 **Sonno insufficiente il {data_sonno}**: solo {durata_sonno:.1f} ore (minimo raccomandato: 7 ore)"
                                    attivita_problematiche.append(problema)
                                    st.error(problema)
                                elif durata_sonno > 9:
                                    st.success(f"💤 **Sonno lungo**: {durata_sonno:.1f} ore - ottimo recupero!")
                        else:
                            st.warning("❌ FUORI periodo registrazione")
